...
We know (from Chapter~\ref{chapter:OMPoverview} that variables cannot appear in more than one data sharing clause\footnote{Other than appearing in both \Code{firstprivate} and \Code{lastprivate}.}.
...
To see this in action, let's look back at our program which computed Pi from earlier (Figure~\ref{code:PiLoop}).
...
\end{Verbatim}
Here, the data array \Code{A} will be copied to and from the device \emph{every} iteration.
The host doesn't need the data until all the iterations (the \Code{it} for-loop) are completed.
...
We can solve this problem by using the OpenMP target data constructs.
...
We will describe the \emph{device data environment} and how to interact with it with the \Code{target data} and \Code{target update} constructs.
...
When we map variables to and from the device (using \Code{map} clauses) at the beginning and end of target regions, they are placed into this device data environment.
This means that they are available for use during the target region.
We can use the \Code{target data} constructs to map variables into the device data environment \emph{without} also transferring execution to the device as it would for the \Code{target} directive.
...
We'll begin with the simplest way to keep data resident on the device between target regions by showing the \Code{target data} directive.
...
The code in Figure~\ref{code:targetDataRegion} shows a simple program that initalises two arrays, and adds one to the other.
...
The first, lines 7--9, updates array \Code{A}.
It's a simplistic example, so the update isn't very complicated, but you could imagine this update was more involved.
The second, lines 11--13, updates array \Code{B}.
The third, lines 15--17, adds array \Code{B} to array \Code{A} element-wise.
Each of these is performed in turn, and the host waits for each target region to execute.

Notice how there are no \Code{map} clauses on any of these target directives.
This means that all the variables will be mapped by the implicit rules (scalars as \Code{firstprivate}, etc.).
So how do we make sure the arrays \Code{A} and \Code{B} are copied to the device?
We do this on line 5, where the map clause is used on the \Code{target data} construct.
This creates a data environment on the target for the duration of the scope, that is lines 5--18.
The map clauses behaves just the same as it does for a \Code{target}, but execution isn't transferred to the device: just the data is copied.
...
Note that the host is not allowed to access these arrays during the scope of the target data region.
