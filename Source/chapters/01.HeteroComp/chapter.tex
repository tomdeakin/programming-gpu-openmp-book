...
moving the fastest and those trying to follow the motion of the pendulum must struggle to keep up.
...
that emerged with the microprocessor.  In those days (the late 70's and early 80's) a programmer
...
programmers came to think of them selves as software engineers working to make ever more
...
truly the job of the hardware and we all rode the Moore's law wagon into a bright future of
...
the transition to multicore chips pulled it into the mainstream.  This was great for 
...
Understanding hardware and using that understanding to write performant code was with us.
...
Low latency computing is difficult.  Since the memory (DRAM) is much slower than the cores that make up the CPU, 
the CPU utilizes a cache hierarchy to maintain a window of DRAM memory.  As long as the items needed from memory 
reside in cache, all is well and low latencies are provided.  If needed data is not in cache, it can become difficult for the
system to deliver the required latencies.

One solution to this problem i a CPU is to exploit concurrency.  When multiple units of work (usually called a ``thread'' on a CPU) are 
available to be scheduled for execution and they are unordered with respect to each other, we say those units of work are concurrent.  
Since the detailed order of instructions is not defined for concurrent threads, the system can swap form one thread to the next so a 
thread waiting on some resource (such as a fresh cache-line from memory) is replaced by another thread that is ready to go.

The result is clear when you look at a block diagram of a CPU.  We show a typical dual socket CPU based in in figure~\ref{}.
To maintain low latencies from threads spread out among the core, multiple layers of cache are needed. Each core has a L1 instruction and L1
data cache.  A core also has a second level unified cache that holds both data an instruction.  Then all the cores in each socket share
a third level cache with is tiled so each core has a local block of L3  cache.  This complexity uses up a lot of Realestate on a chip.  Transistors that could 
have been used for compute are used for caches, translation look-aside buffers to manage virtual memory, coherence interfaces to the 
second socket, and more.   

This is the host processor.  Introduce Cache coherent shared memory machine. SMP model.
