...
Advanced Vector Extensions (AVX) introduced in 2008.   compile as \Code{gcc -mavx}.  The two character "pd" means packed double.
\begin{itemize}
\item \Code{ __m256d}: The type for the 256 bit wide vector register to hold (in our case) four 64-bit floating point numbers.
\item \Code{_mm256_loadu_pd()}:  loads 4 doubles from an address that may not be aligned on a 32 byte boundary.
\item \Code{_mm256_storeu_pd()}:  stores 4 doubles to an address that may not be aligned on a 32 byte boundary.
\item \Code{_mm256_add_pd()}: adds  two vector registers holding floating point numbers to return 
their sum in another vector register.
\end{itemize}

The above operations assume the data is unaligned.  For best efficiency, the arrays \Code{A, B, C} should be 
